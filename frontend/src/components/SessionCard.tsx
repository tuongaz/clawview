  ...
  return (
    ...
    >
      <Card.Header className="flex-row items-center gap-2 px-4 pt-3 pb-0">
        ...
        {session.name ? (
          <>
            <span className={`font-semibold text-sm truncate max-w-[160px] ${isActive ? 'text-[var(--text-bright)]' : 'text-[var(--text-primary)]'}`}>
              {session.name}
            </span>
            <span className="text-[var(--text-secondary)] font-mono text-[13px]">
              {slug}
            </span>
          </>
        ) : (
          <span className={`font-mono font-semibold text-sm tracking-wider ${isActive ? 'text-[var(--text-bright)]' : 'text-[var(--text-secondary)]'}`}>
            {slug}
          </span>
        )}
        <span className="ml-auto text-[var(--text-secondary)] text-[13px] font-mono">
      ...
      <Card.Content className="flex flex-col gap-2.5 px-4 py-2">
        {session.lastUserPrompt && (
          <div className="text-[var(--text-primary)] text-sm leading-[1.4] whitespace-pre-wrap break-words line-clamp-3">
            {session.lastUserPrompt.startsWith('/') ? (
              <><span className="text-[13px] opacity-60 mr-0.5">&#8984;</span> {session.lastUserPrompt}</>
            ) : (
              session.lastUserPrompt
            )}
          </div>
        ...
        {session.lastAction && (
          <div className="flex items-center gap-1.5 text-warning text-[13px] font-mono bg-warning/[0.08] py-1 px-2.5 rounded-[var(--radius-sm)] line-clamp-2 break-words">
            {isActive && !isWaiting ? (
              <ActiveDot />
            ) : (
